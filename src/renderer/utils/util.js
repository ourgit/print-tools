 ...
 */
export function generateSingleCode(issueId, orgId, seq) {
    if (!issueId || !orgId || !seq) return "";
    var date = getTodayDate();
    seq = seq.toUpperCase();
    var vcode = generateVcode(issueId, orgId, date, seq);

    return issueId + "" + orgId + "" + date + "" + seq + "" + vcode;
 ...
 * @param {*} seq 序列号
 */
function generateVcode(strcode) {
    var sum = 0;
    for (var i = 0; i < strcode.length; i++) {
        var str = strcode.chatAt(i);
    ...
    return getSecondMapCode(modValue);
}
